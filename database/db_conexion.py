...
class DatabaseManager:
    ...
    def obtener_preferencias(self, id_profesor=None):
        try:
            query = self.client.table('preferencias').select("*")
            if id_profesor:
                query = query.eq('id_trabajador', id_profesor)
    ...
    # Generador de datos para generación de horarios
        ...
        try:
            ...
            for m in modulos:
                item = {
                    ...
                    "max_diarias": m.get('horas_max_dia', 2),
            ...
            for h in raw:
                ...
                if modo == 'CLASE' and h.get('ciclo') == filtro:
                    ...
                    if f"{prof['nombre']} {prof['apellidos']}" == filtro:
                        ...
                        t2 = f"Aula: {h.get('ciclo')}" if mod else ""
                        col = prof['color_asignado']

                if match:
                    resultado.append({"dia": h['dia_semana'], "hora": h['franja_horaria'], "texto1": t1, "texto2": t2, "color": col})
            return resultado
        except: return []
